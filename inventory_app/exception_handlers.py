...
from . import ItemNotFound, ItemAlreadyExists
...
def add_exception_handlers(app: FastAPI) -> None:
    ...
    async def item_exists_exception_handler(request: Request, exc: ItemAlreadyExists):
        logger.error(f"Item with id {exc.item_id!r} already exists")
        return JSONResponse(
            status_code=status.HTTP_409_CONFLICT,
            content={"message": f"An item with the id {exc.item_id!r} is already in the database", "item_id": None}
        )