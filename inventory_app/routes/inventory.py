...
from ..schemas.item import Item, MultipleItemsResponse, CreateUpdateItemResponse
...
def create_inventory_router(
    ...
    # This route needs to be here otherwise the request '/inventory/main' will go through 
    # '/inventory/{inventory_id}' which will generate an error
    @inventory_router.get('/main', response_model=MultipleItemsResponse, status_code=status.HTTP_301_MOVED_PERMANENTLY)
    async def get_main_inventory():
    ...
    async def get_items_from_inventory(inventory_id: int, start: int = 0, limit: int = 3):
        """
        ...
        items, total = await inventory_service.get_items_with_pagination(inventory_id, start, limit)
    ...
    async def add_item_to_inventory(inventory_id: int, item: Item):
        ...
        message, item_id = await inventory_service.add_item_to_inventory_db(inventory_id, item)
